...
3. Определённость (детерминированность — обязательное завершение каждого из действий). На каждом этапе выполнения алгоритма должно быть понятно, что именно нужно делать. Для выполнения алгоритма могут потребоваться входные данные, которые используются для получения выходного результата. Эти данные должны быть чётко определены.
4. Однозначность: каждый шаг алгоритма должен быть чётко определён и не допускать двусмысленности.
5. Конечность (результативность). Алгоритм должен завершаться после выполнения определённого числа шагов. Это означает, что он не может выполняться бесконечно и должен иметь чётко определённый конец. Алгоритм должен давать хотя бы один выходной результат после выполнения всех шагов. Этот результат может быть значением, данными или состоянием системы.
6. Выполнимость (эффективность)
7. Массовость (универсальность — возможность применения для целого класса задач). Алгоритм должен быть применимым ко множеству задач одного типа, то есть он должен быть универсальным и пригодным для решения схожих задач.
