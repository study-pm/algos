...
- [21. Методы в С#](#21-методы-в-с)
  ...
  - [Параметры методов](#параметры-методов)
    ...
    - [Передача параметров по ссылке и модификатор ref](#передача-параметров-по-ссылке-и-модификатор-ref)
    - [Выходные параметры. Модификатор `out`](#выходные-параметры-модификатор-out)
    - [Модификатор `params`](#модификатор-params)
    - [Массив в качестве параметра](#массив-в-качестве-параметра)
...
- [24. Строки в языке С#. Функции для работы со строками.](#24-строки-в-языке-с-функции-для-работы-со-строками)
  ...
  - [Неизменяемые строки `string`](#неизменяемые-строки-string)
    ...
    - [Методы работы со строками](#методы-работы-со-строками)
    - [Вывод служебных символов](#вывод-служебных-символов)
...
- [26. Рекурсивные функции.](#26-рекурсивные-функции)
  ...
  - [Важные моменты при работе с рекурсией](#важные-моменты-при-работе-с-рекурсией)
- [27. Область видимости переменных. Локальные и глобальные переменные. Статические и динамические переменные](#27-область-видимости-переменных-локальные-и-глобальные-переменные-статические-и-динамические-переменные)
  ...
  - [Глобальные переменные (члены класса)](#глобальные-переменные-члены-класса)
...
- [30. Структуры в языке С#. Определение и инициализация структуры. Создание экземпляра структуры.](#30-структуры-в-языке-с-определение-и-инициализация-структуры-создание-экземпляра-структуры)
  ...
  - [Структуры](#структуры)
    - [Определение и инициализация структуры](#определение-и-инициализация-структуры)
    - [Создание экземпляра структуры](#создание-экземпляра-структуры)
    - [Инициализация структуры](#инициализация-структуры)
    - [Особенности структур](#особенности-структур)
...
- [31. Базовые понятия ООП: объект, его свойства и методы, класс. Основные принципы ООП: инкапсуляция, наследование, полиморфизм.](#31-базовые-понятия-ооп-объект-его-свойства-и-методы-класс-основные-принципы-ооп-инкапсуляция-наследование-полиморфизм)
  - [Базовые понятия ООП](#базовые-понятия-ооп)
  - [Основные принципы ООП](#основные-принципы-ооп)
- [32. Классы в С#](#32-классы-в-с)
  - [Основные понятия](#основные-понятия-1)
    - [Определение класса](#определение-класса)
    - [Модификаторы (спецификаторы)](#модификаторы-спецификаторы)
    - [Создание экземпляра класса](#создание-экземпляра-класса)
    - [Члены (элементы) класса](#члены-элементы-класса)
  - [Доступ к членам объекта](#доступ-к-членам-объекта)
  ...
  - [Наследование конструкторов. Ключевое слово `base`](#наследование-конструкторов-ключевое-слово-base)
  ...
  - [Полиморфизм: виртуальные методы, переопределение методов в прозиводных классах](#полиморфизм-виртуальные-методы-переопределение-методов-в-прозиводных-классах)
...
4. **Однозначность** (**единственность толкования**)
...
Пусть выражение включает два операнда. Если оба операнда имеют одинаковый тип, то и выражение будет этого же типа. Если операнды имеют различный тип, то тип выражения будет соответствовать «старшему» типу, т.е. система берет на себя преобразование младшего типа к старшему перед выполнением операции. Старшинство типов в порядке возрастания: `sbyte`, `short`, `int`, `long`, `float`, `double`.

Арифметические операции не определены для более коротких, чем `int` типов. Для таких величин перед выполнением операций осуществляется преобразование в тип `int`.

Если переменная слева от знака присваивания имеет тот же тип, что и у выражения справа от знака присваивания, то происходит простое присваивание переменной нового значения.

Если переменная слева от знака присвоения имеет более старший тип, чем тип выражения справа, то перед присвоением значения производится автоматическое изменение типа.

Если переменная слева имеет более младший тип, чем выражение справа, то при компиляции выдается сообщение об ошибке. В такой ситуации требуется явное преобразование типа.

Явное изменение типа выражения для простой числовой переменной осуществляется с помощью операции приведения типа
```c#
int n2=123;
double d4= 17.6;
n1=n2+(int)d4;
n1=(int)(n2+d4);
```
Операция приведения типа задается записью в круглых скобках перед переменной или выражением имени нового типа. Язык С# не разрешает *простое* преобразование любых типов в тип `bool` или преобразование типа `bool` в другие типы.

Явное приведение типа можно осуществлять как в сторону повышения, так и в сторону понижения типа.

Явное преобразование можно осуществлять с помощью специальных команд:
```c#
Convert.ToInt32(val)
Convert.ToInt64(val)
...
```
и др. Здесь вместо `val` может стоять переменная или выражение любого допустимого типа.

Такой способ преобразования применяют при вводе числовой информации с клавиатуры (в этом случае `val` представляет собой строку).

Для переменной булевого типа:
- `true`, если `val` не равно 0;
- `false`, если `val` равно 0.

В ходе преобразования типа возможны проблемы:
- преобразование из типов `int`, `long` в тип `float` и из типа `long` в тип `double` может вызвать потерю точности (количество знаков в мантиссе вещественного числа меньше количества разрядов, отводимых для представления целых чисел);
- преобразование вещественных чисел в целые может приводить к возникновению ошибки времени выполнения, так как диапазон вещественных чисел существенно больше диапазона целых величин;
- переменная `val` может содержать нечисловые данные, набранные пользователем.
...
можно инициализировать (присвоить ей начальное значение), а затем в любой момент ей можно присвоить новое значение, которое заменит собой предыдущее.
...
**Константа**, в отличие от переменной, не может менять свое значение. Константы в C# представляют значения, которые не могут быть изменены после их объявления и инициализации. Для объявления константы используется ключевое слово `const`.
...
Константы бывают трех видов: *литералы*, *типизированные константы* и *перечисления*.
...
В языке C# операции и выражения используются для выполнения вычислений и логических проверок. Операции могут быть арифметическими, логическими, а также включать основные математические функции. Рассмотрим каждую из этих категорий подробно.
...
Арифметические операции в C# включают сложение, вычитание, умножение, деление и взятие остатка. Эти операции могут быть выполнены над числами различных типов, таких как `int`, `float`, `double` и другие.
...
int sum = a + b;       // Сложение: sum = 15
int difference = a - b; // Вычитание: difference = 5
int product = a * b;    // Умножение: product = 50
int quotient = a / b;   // Деление: quotient = 2
int remainder = a % b;  // Остаток: remainder = 0
```

#### Логические операции
Логические операции используются для выполнения логических проверок и объединения логических выражений. Основные логические операции в C# включают `&&` (логическое И), `||` (логическое ИЛИ) и `!` (логическое НЕ).
...
bool andResult = x && y;   // Логическое И: andResult = false
bool orResult = x || y;    // Логическое ИЛИ: orResult = true
bool notResult = !x;       // Логическое НЕ: notResult = false
```

#### Операции сравнения
Операции сравнения используются для сравнения значений и возвращают логический результат (`true` или `false`).
...
bool isEqual = a == b;    // Равно: isEqual = false
bool isNotEqual = a != b; // Не равно: isNotEqual = true
bool isGreater = a > b;   // Больше: isGreater = true
bool isLesser = a < b;    // Меньше: isLesser = false
bool isGreaterOrEqual = a >= b; // Больше или равно: isGreaterOrEqual = true
bool isLesserOrEqual = a <= b;  // Меньше или равно: isLesserOrEqual = false
```
...
Примеры основных математических функций:
```c#
using System;
...
class MathExamples
{
    static void Main()
    {
        ...
        double sqrt = Math.Sqrt(x);        // Квадратный корень: sqrt = 3.0
        double pow = Math.Pow(x, y);       // Возведение в степень: pow = 81.0
        double abs = Math.Abs(-x);         // Модуль числа: abs = 9.0
        double sin = Math.Sin(Math.PI / 2);// Синус: sin = 1.0
        double cos = Math.Cos(0);          // Косинус: cos = 1.0
        double tan = Math.Tan(Math.PI / 4);// Тангенс: tan = 1.0
        double log = Math.Log(10);         // Логарифм: log ≈ 2.302
        double exp = Math.Exp(1);          // Экспонента: exp ≈ 2.718
        double round = Math.Round(3.14159, 2); // Округление: round = 3.14
...
устройствами. Функция ввода в языке C# используется для получения данных от пользователя во время выполнения программы.  Совокупность стандартных устройств ввода (клавиатура) и вывода (экран) называется **консолью**. В языке С# нет операторов ввода и вывода. Вместо них для обмена данными с внешними устройствами используются специальные классы. В частности, для работы с консолью используется стандартный класс `Console`, определенный в пространстве имен `System`.
...
метод `Parse()`, который реализован для всех числовых типов данных. Таким образом, если нам потребуется преобразовать строковое представление в вещественное, мы можем воспользоваться методом `float.Parse()` или `double.Parse()`. В случае, если соответствующее преобразование выполнить невозможно, то выполнение программы прерывается и генерируется исключение. Например, если входная строка имела неверный формат, то будет сгенерировано исключение `System.FormatExeption`.

При преобразовании строк в числа может возникнуть исключение, если пользователь введет некорректные данные. Чтобы избежать сбоев в программе, рекомендуется использовать метод `int.TryParse`, который позволяет безопасно выполнять преобразование.
...
Функции вывода в языке C# используются для отображения информации на консоли. Основные методы для вывода данных включают `Console.Write` и `Console.WriteLine`, которые позволяют выводить текст, переменные и результаты вычислений. Рассмотрим эти методы подробнее.

-	`Console.Write` — выводит текст без перехода на новую строку.
-	`Console.WriteLine` — выводит текст с переходом на новую строку.
...
4. `Console.WriteLine($"x={x} y={y}");`: с использованием интерполяции строк.
...
Условные операторы в языке C# позволяют выполнять различные действия в зависимости от выполнения определённых условий. Основными условными операторами в C# являются `if`, `else if`, `else`, и тернарный оператор `? :`.

### Операторы `if`, `else if`, `else`
Оператор `if` выполняет блок кода, если условие истинно (`true`). Если условие ложно (`false`), выполнение этого блока кода пропускается. Оператор `else` используется для выполнения блока кода, если условие оператора `if` ложно. Оператор `else if` позволяет проверять дополнительные условия, если предыдущее условие оператора `if` ложно.
...
### Тернарный оператор
Тернарный оператор `? :` позволяет записать условное выражение в компактной форме. Он возвращает одно значение, если условие истинно, и другое значение, если условие ложно.
...
Оператор выбора в языке C# представлен конструкцией `switch`. Он используется для выполнения различных блоков кода в зависимости от значения выражения. Оператор `switch` позволяет более эффективно и читаемо обрабатывать множественные условия, чем последовательные операторы `if-else if`.
...
Оператор `switch` в C# поддерживает не только целочисленные значения, но и строки, перечисления и другие типы данных. Выражение, стоящее за ключевым словом `switch`, должно иметь арифметический, символьный, строковый тип или тип указатель. Все константные выражения должны иметь разные значения, но их тип должен совпадать с типом выражения, стоящего внутри скобок `switch` или приводиться к нему. Ключевое слово `case` и расположенное после него
...
Перечисления (`enum`) часто используются вместе с оператором `switch` для улучшения читаемости кода и избежания ошибок.
```c#
using System;
...
Операторы цикла используются для организации многократно повторяющихся вычислений и позволяют повторять выполнение блока кода несколько раз. К операторам цикла относятся: цикл с предусловием `while`, цикл с постусловием `do while`, цикл с параметром `for` и цикл перебора `foreach`.

### Цикл с предусловием `while`
Цикл `while` проверяет условие перед каждой итерацией. Если условие истинно, выполняется тело цикла. Цикл продолжается до тех пор, пока условие истинно.
...
### Цикл с постусловием `do while`
Оператор цикла `do while` также организует выполнение одного оператора (простого или составного) неизвестное заранее число раз. Однако в отличие от цикла `while` условие завершения цикла проверяется после выполнения тела цикла. Поскольку цикл `do-while` проверяет условие после каждой итерации, то это гарантирует, что тело цикла выполнится как минимум один раз, даже если условие изначально ложно. Формат цикла `do while`:
...
Цикл `for` используется, когда известно количество итераций заранее. Он включает инициализацию переменной, условие и обновление переменной в одной строке.
...
```
*Инициализация* (*initializer*) используется для объявления и/или присвоения начального значения величине, используемой в цикле в качестве параметра (счетчика). Областью действия переменной, объявленной в части инициализации цикла, является цикл и вложенные блоки. Инициализация выполняется один раз в начале исполнения цикла.

*Выражение* (*loop condition*) определяет условие выполнения цикла: если его результатом является истина, цикл выполняется. Истинность выражения проверяется перед каждым выполнением тела цикла, таким образом, цикл с параметром реализован как цикл с предусловием.

*Модификация* (*iterator*) выполняется после каждой итерации цикла и служит обычно для изменения *параметра* (*control variable*) цикла.
...
Цикл `foreach` используется для итерации по коллекциям, таким как массивы, списки и другие коллекции, реализующие интерфейс `IEnumerable`. Он удобен для перебора всех элементов коллекции, не беспокоясь о границах или индексах, а также о порядке обхода.
...
Оператор `break` используется внутри операторов цикла и оператора выбора для обеспечения перехода в точку программы, находящуюся непосредственно за оператором, внутри которого находится `break`. Оператор `break` немедленно завершает выполнение ближайшего внешнего цикла или оператора `switch`, в котором он находится, и передает управление следующей инструкции после завершенного цикла или оператора.
...
Оператор перехода к следующей итерации цикла `continue` пропускает все операторы, оставшиеся до конца тела цикла, и передает управление на начало следующей итерации (повторение тела цикла). Таким образом, оператор `continue` пропускает оставшийся код в текущей итерации и передает управление следующей итерации цикла. Рассмотрим оператор `continue` на примере.
```c#
static void Main()
{
 Console.WriteLine("n=");
...
Методы в C# позволяют структурировать и организовывать код в логические блоки, которые выполняют определенные задачи. Они помогают избегать дублирования кода, повышают читаемость и поддерживаемость программ. **Метод** — это блок кода, который выполняет определенную задачу. Методы могут принимать параметры и возвращать значения.
...
Метод состоит из *сигнатуры* и *тела*. **Сигнатура** включает в себя модификаторы, возвращаемый тип, имя метода и параметры. **Тело** метода содержит код, который выполняется при вызове метода
...
Методы могут иметь различные модификаторы доступа и поведения, которые определяют, как и где они могут быть использованы.
- `public`: Метод доступен из любого места в программе.
- `private`: Метод доступен только внутри текущего класса.
- `protected`: Метод доступен внутри текущего класса и производных классов.
- `internal`: Метод доступен внутри текущей сборки.
- `static`: Метод принадлежит классу, а не экземпляру класса.
...
class Program
{
    static void Main()
    {
        Console.WriteLine(Sum(5, 3)); // Вызов статического метода
        Program p = new Program();
        Console.WriteLine(p.Multiply(5, 3)); // Вызов нестатического метода
    }
...
Основными способами передачи параметров в метод в языке C# является передача по значению и по ссылке.
...
При передаче **параметра по значению** метод получает копии параметров, и операторы метода работают с этими копиями (независимо от самой переменной). Доступа к исходным значениям параметров у метода нет, а, следовательно, нет и возможности их изменить.
...
При передаче **параметров по ссылке** метод получает копии адресов параметров, что позволяет осуществлять доступ к ячейкам памяти по этим адресам и изменять исходные значения параметров. И, таким образом, если в методе изменяется значение параметра, передаваемого по ссылке, то также изменяется и значение переменной, которая передается на его место. Так, в метод `Increment` передается ссылка на саму переменную number в памяти. И если значение параметра `n` в `Increment` изменяется, то это приводит и к изменению переменной `number`, так как и параметр `n` и переменная `number` указывают на один и тот же адрес в памяти.

Для того чтобы параметр передавался по ссылке, необходимо при описании метода перед формальным параметром и при вызове метода перед соответствующим фактическим параметром поставить служебное слово `ref`. Обратите внимание, что модификатор `ref` указывается как перед параметром при объявлении метода, так и при вызове метода перед аргументом, который передается параметру.
```c#
class Program
  {
...
class Program
{
    static void Main()
    {
        Console.WriteLine(Add(5, 3)); // Вызов метода Add с двумя параметрами
        Console.WriteLine(Add(1, 2, 3)); // Вызов метода Add с тремя параметрами
    }
...
### Рекурсивные методы
Рекурсивный метод вызывает сам себя для решения подзадачи. Важно иметь условие выхода, чтобы предотвратить бесконечную рекурсию.
...
**Массив** в C# — это структура данных, которая позволяет хранить несколько элементов одного типа под одним именем. Элементы массива доступны по индексу, начиная с нуля. Массивы удобны для хранения и обработки коллекций данных, таких как числа, строки или объекты.

**Массив** — набор элементов одного и того же типа, объединенных общим именем.  Массивы в С# с точки зрения синтаксиса практически не отличаются от массивов в С, C++ и Java и могут быть использованы по аналогии с тем, как они используются в других языках программирования. Однако С#-массивы имеют существенные отличия: они относятся к ссылочным типам данных, более того — реализованы как объекты. Внутренне массив в С# устроен как тип, производный от класса `System.Array`. Фактически имя массива является ссылкой на область кучи (динамической памяти), в которой последовательно размещается набор элементов определенного типа. Выделение памяти под элементы происходит на этапе инициализации массива. А за освобождением памяти следит система сборки мусора — неиспользуемые массивы автоматически утилизируются данной системой.
...
Чтобы объявить массив, нужно указать тип элементов, квадратные скобки и имя массива:
```c#
тип[] имя_массива;
```

Инициализация массива может быть выполнена одновременно с его объявлением или отдельно. Существует несколько способов инициализации массивов в C#.
```c#
int[] numbers = new int[5];
// Инициализация при объявлении с указанием значений:
int[] numbers = new int[] { 1, 2, 3, 4, 5 };
```

Заполнить массив можно, перечисляя элементы последовательно в фигурных скобках.  А можно использовать для этой цели числовые индексы.
...
Обращения к элементам массива происходит с помощью индексов (начиная с 0), для этого нужно указать имя массива и в квадратных скобках его номер. Например, `a[0]`, `b[10]`, `c[i]`.
...
`BinarySearch()` |	статический метод |	Двоичный (бинарный) поиск в отсортированном массиве
`Clear()` |	статический метод | Очистка массива (присваивание всем элементам массива значений по умолчанию)
...
`Exists()` | статический метод | Проверят, содержит ли массив определенный элемент (по предикату)
`Find()` | статический метод | Находит элемент, который удовлетворяет определенному условию
`FindAll()` | статический метод | Находит все элементы, которые удовлетворяют определенному условию
`GetValue()` |	экземплярный метод |	Получение значения элемента массива
`IndexOf()` | статический метод |	Поиск первого вхождения элемента в одномерный массив (возвращает индекс элемента)
`LastIndexOf` |	статический метод |	Поиск последнего вхождения элемента в одномерный массив
`Rank` | свойство |	Возвращает размерность массива (`1` — одномерный, `2` — двумерный и т.п.)
`Resize()` |	статический метод | Изменяет размер одномерного массива
`Reverse()` |	статический метод |	Изменение порядка следования элементов на обратный (располагает элементы массива в обратном порядке)
`SetValue()` |	экземплярный метод |	Установка значения элемента массива
`Sort()` |	статический метод |	Упорядочивание элементов одномерного массива (сортировка)
...
Второй тип многомерного массива можно назвать “ломаным” ("рваным", ступенчатым или зубчатым — jagged). Такой массив содержит в качестве внутренних элементов некоторое количество внутренних массивов, каждый из которых может иметь свой внутренний уникальный размер.
```c#
// "Ломаный" многомерный массив (массив из массивов).
// В нашем случае - это массив из
// пяти внутренних массивов разного размера
...
//мы помним, каждому элементу присваивается значение по умолчанию - в нашем случае 0)
for (int i = 0; i < 5; i++)
{
...
### Многомерные массивы
**Многомерные массивы** представляют собой массивы, которые имеют более одного измерения. В C# можно объявлять двумерные массивы (**матрицы**) и массивы с большим числом измерений. Чаще всего используются двумерные массивы, которые представляют собой таблицы. Каждый элемент массива имеет два индекса, первый определяет номер строки, второй — номер столбца, на пересечении которых находится элемент. Нумерация строк и столбцов начинается с нуля.
...
Инициализация двумерного массива может быть выполнена одновременно с его объявлением или отдельно. Существует несколько способов инициализации многомерных массивов в C#. Например:
```c#
int [,] a;
int [,] a= new int [3, 4];
int [,] a={{0, 1, 2}, {3, 4, 5}};
...
Доступ к элементам многомерного массива осуществляется с использованием индексов для каждого измерения, начиная с 0. Обращение в двумерном массиве:

- `arr[i,j]` – получить элемент в i-й строке и j-м столбце
...
Как правило, прямоугольный массив перебирается с помощью цикла `for`. Измерение массива влияет на уровень вложенности цикла. Например, для перебора одномерного массива используется 1 цикл `for`, для двумерного 1 вложенный цикл `for`, для 3-мерного 2 вложенных `for` и т.д.

### Ступенчатые (зубчатые) массивы
В ступенчатых массивах (jagged arrays) количество элементов в разных строках может быть различным. В памяти ступенчатый массив хранится в виде массива массивов. Структура ступенчатого массива:
...
Строки (типа `string`) в C# представляют собой последовательности символов. 
Тип `string`, предназначенный для работы со строками символов в кодировке Unicode, является встроенным типом С#. Ему соответствует базовый тип класса `System.String` библиотеки .Net. Каждый объект `string` — это неизменяемая последовательность символов Unicode, т.е. методы, предназначенные для изменения строк, возвращают измененные копии, исходные же строки остаются неизменными. Это неизменяемый (immutable) тип данных, что означает, что после создания строки её значение не может быть изменено. Вместо этого, любые операции, которые кажутся изменяющими строку, создают новую строку.
...
4. Cимвол `@` сообщает конструктору `string`, что строку нужно воспринимать буквально, даже если она занимает несколько строк:
    ```c#
    string s = @"Привет!
...
10. Интерполяция строк (с версии C# 6.0):
    ```c#
    string name = "Alice";
    string greeting = $"Hello, {name}!";
    ```

Доступ к отдельным символам строки можно получить с помощью индексации.

```c#
stringstr= "test"; Console.WriteLine(str[0]);
```
...
`Contains()` |	Экземплярный метод |	Проверяет, содержит ли строка указанную подстроку.
...
`Format()` |	Статический метод |	Форматирование строки в соответствии с заданным форматом — с использованием других примитивов числовых данных, других строк) и подстановочных выражений вида `{0}`.
`IndexOf()`, `IndexOfAny()`, `LastIndexOf()`, `LastIndexOfAny()` |	Экземплярные методы	| Определение индексов первого и последнего вхождения заданной подстроки или любого символа из заданного набора в данную строку. Возвращают индекс вхождения искомой подстроки или -1, если она не будет обнаружена.
`Insert()` |	Экземплярный метод |	Вставка подстроки в заданную позицию внутри существующей.
...
`Split()` |	Экземплярный метод |	Разделяет строку на элементы, используя разные разделители. Результаты помещаются в массив строк. Используется с опцией `RemoveEmptyEntries` и/или `TrimEntries`.
...
`ToLower()`, `ToUpper()` |	Экземплярные методы |	Преобразование строки к нижнему или верхнему регистру. Эти методы используются для получения копии строки, в которой все символы станут строчными или прописными.
...
Помимо управляющих последовательностей, в С# предусмотрен также специальный префикс `@` для дословного вывода строк вне зависимости от наличия в них управляющих последовательностей. Строки с этим префиксом называются “дословными”(**verbatims trings**). Это — очень удобное средство во многих ситуациях:
```c#
// Префикс @ отключает обработку
// управляющих последовательностей
stringfinalString-@"\n\tStringfile: 'C:\CSnarpProjects\Strings\string.cs'";
...
При работе со строками в С# необходимо помнить очень важную вещь: **значение строки не может быть изменено**. Все методы, казалось бы, изменяющие строку, на самом деле лишь возвращают её измененную копию.
```c#
// Вносим изменения в строку? На самом деле нет...
System.StringstrFixed= "ThisishowI beganlife";
Console.WriteLine(strFixed);
stringupperVersion= strFixed.ToUpper();
// Возвращает "прописную" копию strFixed
Console.WriteLine(strFixed);
Console.WriteLine(upperVersion);
```

Чтобы создать строку, которую можно изменять, в С# предусмотрен класс `StringBuilder`, определенный в пространстве имен `System.Text`. Объекты этого класса всегда объявляются с явным вызовом конструктора класса (через операцию `new`). Все изменения, которые вы вносите в объект этого класса, немедленно в нем отражаются, что во многих ситуациях гораздо эффективнее, чем работать с множеством копий.
```c#
...
classStringApp
{
  publicstaticintMain(stnng[] args)
  {
    // Создаем объект StringBuilderи изменяем его
    //содержимое
    StringBuildermyBuffer= newStringBuilder(“I ama  buffer”);
    myBuffer.Append(“thatjustgotlonger...”);
...
publicstaticintMain(string[] args)
  {
    ...
    myBuffer.Append(" thatjustgotlonger...");
    Console.WriteLine(myBuffer) ;
    myBuffer.Append(“andevenlonger.”);
    Console. WriteLine(myBuffer);
    // Делаем все буквы прописными
    stringtheReallyFinalString= myBuffer.ToString(). ToUpper();
...
**Функции** (или **методы**) в C# — это блоки кода, которые выполняют определённую задачу. Они позволяют структурировать код, делают его более читаемым и упрощают повторное использование кода.

https://uchet-jkh.ru/i/procedura-i-funkciya-sut-i-otliciya
...
### Структура функции
Функция в C# состоит из следующих элементов:
1. Модификаторы доступа (например, `public`, `private`).
2. Возвращаемый тип (например, `void`, `int`, `string`).
3. Имя функции (например, `MyFunction`).
4. Список параметров в круглых скобках (например, `(int a, int b)`).
...
### Вызов функции
Чтобы использовать функцию, её нужно вызвать из другой функции или метода. Для этого используется имя функции и передаются необходимые параметры.
```c#
class Program
{
    static void Main()
...
### Прототип функции
**Прототип функции** (или **объявление функции**) — это определение функции, которое включает её сигнатуру (имя, возвращаемый тип и параметры), но не содержит тела. В C# прототипы используются в интерфейсах и абстрактных классах.
...
**Рекурсия** — это метод программирования, при котором функция вызывает сама себя для решения подзадачи той же проблемы. Рекурсивные функции полезны для решения задач, которые можно разделить на подзадачи, аналогичные исходной задаче, такие как вычисление факториала, числа Фибоначчи, обход деревьев и многие другие.
...
1. **Базовый случай**: Очень важно определить корректный базовый случай, иначе функция будет вызывать саму себя бесконечно.
2. **Рекурсивный случай**: Необходимо убедиться, что каждый рекурсивный вызов приближает задачу к базовому случаю.
3. **Производительность**: Рекурсивные функции могут быть неэффективны из-за большого числа вызовов и использования стека вызовов. В некоторых случаях рекурсию можно заменить итерацией для улучшения производительности.
4. **Хвостовая рекурсия**: Это особый случай рекурсии, когда рекурсивный вызов является последней операцией функции. Некоторые компиляторы могут оптимизировать хвостовую рекурсию для улучшения производительности.
...
Область видимости переменной определяет, где в коде эта переменная может быть использована. В C# существуют несколько уровней видимости переменных, включая *локальные*, *глобальные* (членов класса) и *статические* переменные.
...
Локальные переменные объявляются внутри методов, конструкторов или блоков кода и доступны только внутри этих методов или блоков. Они создаются при вызове метода и уничтожаются при завершении метода.
...
class Program
{
    ...
    static void PrintLocalVar()
    {
        int anotherLocalVar = 20; // Другая локальная переменная
        Console.WriteLine(anotherLocalVar);
    }
}
```

### Глобальные переменные (члены класса)
**Глобальные переменные** — это поля, объявленные в пределах класса, но вне всех методов. Они доступны для всех методов этого класса. Их также называют **членами класса**.
...
class Program
{
    static int globalVar = 10; // Глобальная переменная
...
Статические переменные объявляются с использованием ключевого слова `static` и принадлежат классу, а не экземпляру класса. Это означает, что все экземпляры класса разделяют одно и то же значение этой переменной. Статические переменные и методы могут быть вызваны без создания экземпляра класса.
...
class Program
{
    ...
    static void UpdateStaticVar()
    {
        staticVar = 50; // Изменение статической переменной
    }
}
```

### Динамические переменные
В C# нет прямого аналога "динамических переменных" в смысле их области видимости, но существует ключевое слово `dynamic`, которое позволяет переменным менять тип во время выполнения. Оно используется для создания переменных, тип которых определяется во время выполнения, а не компиляции.
...
class Program
{
    static void Main()
    {
        dynamic dynamicVar = 10; // Переменная типа int
        Console.WriteLine(dynamicVar);

        dynamicVar = "Hello, world!"; // Теперь переменная типа string
        Console.WriteLine(dynamicVar);
    }
}
```

Более подробно о ключевом слове `dynamic` изложено дальше.

### Обобщение
- **Локальные переменные**: Определены внутри методов или блоков кода, видимы только в пределах этих методов или блоков.
- **Глобальные переменные**: Поля класса, видимые для всех методов этого класса.
- **Статические переменные**: Поля класса, общие для всех экземпляров класса, могут быть использованы без создания экземпляра.
- **Динамические переменные**: Переменные, тип которых определяется во время выполнения с использованием ключевого слова `dynamic`.
...
Для ввода данных с консоли в C# используется метод `Console.ReadLine()`. Этот метод читает строку текста, введённую пользователем, и возвращает её в виде строки.

Для вывода данных на консоль в C# используется метод `Console.WriteLine()`, который выводит строку текста и переводит курсор на новую строку, и метод `Console.Write()`, который выводит строку текста без перевода курсора.

Для форматирования строк в C# можно использовать методы `Console.WriteLine()` и `Console.Write()` с форматированием. Форматирование строк позволяет вставлять значения переменных в строки в заданном формате.
- **Composite formatting** — использование фигурных скобок `{}` с индексами `{0}`, `{1}` и т.д.
- **String interpolation** — использование `$` перед строкой и вставка выражений в `{}` внутри строки.
...
Структуры в C# являются типами значений, которые позволяют создавать объекты для хранения набора связанных данных. Структуры могут содержать поля, свойства, методы и конструкторы. Они часто используются для представления небольших объектов, которые не требуют сложных функциональных возможностей классов.
...
#### Определение и инициализация структуры
Определение структуры начинается с ключевого слова `struct`, за которым следует имя структуры и её члены.
...
В этом примере структура `Point` содержит два поля `X` и `Y`, конструктор для инициализации этих полей и метод `Display` для вывода значений полей на консоль.
...
Экземпляры структур могут быть созданы как с использованием конструктора, так и без него. При создании экземпляра структуры без конструктора все поля будут инициализированы значениями по умолчанию (0 для числовых типов).
...
class Program
{
    static void Main()
    {
        // Создание экземпляра структуры с использованием конструктора
        Point p1 = new Point(10, 20);
        p1.Display();

        // Создание экземпляра структуры без использования конструктора
        Point p2;
        p2.X = 30;
        p2.Y = 40;
        p2.Display();
...
Инициализация структуры может быть выполнена как в момент создания экземпляра, так и позже. При инициализации важно помнить, что все поля структуры должны быть инициализированы перед использованием.
...
class Program
{
    static void Main()
    {
        // Инициализация структуры при создании экземпляра
        Rectangle rect1 = new Rectangle(10, 20);
        Console.WriteLine($"Area of rect1: {rect1.GetArea()}");

        // Инициализация структуры после создания экземпляра
        Rectangle rect2;
        rect2.Width = 15;
        rect2.Height = 25;
...
1. **Тип значения**: Структуры являются типами значений, что означает, что они хранят свои данные непосредственно, а не через ссылку, как это делают классы.
2. **Нет наследования**: Структуры не поддерживают наследование, но могут реализовывать интерфейсы.
3. **Иммутабельность**: Для обеспечения неизменяемости структуры можно сделать её поля `readonly`.
4. **Автоматические свойства**: Структуры могут иметь автоматические свойства.
...
**Объектно-ориентированное программирование** (ООП) — это парадигма программирования, которая использует "объекты" для создания программного обеспечения. ООП основано на таких понятиях как "класс", "объект", "интерфейс", "инкапсуляция", "наследование", "полиморфизм", "событие". Основные понятия ООП включают объект, его свойства и методы, а также класс. 
...
**Объект** — это экземпляр класса. Он представляет конкретный элемент в программной системе с уникальным состоянием и поведением. Объекты обладают следующими характеристиками:
- **Состояние** (**свойства**): Данные или атрибуты объекта.
- **Поведение** (**методы**): Функции или действия, которые объект может выполнять.

**Свойства** (также называемые **атрибутами** или **полями**) определяют состояние объекта. Они хранят данные, относящиеся к объекту.

**Методы** — это функции, принадлежащие объекту. Они определяют поведение объекта и позволяют взаимодействовать с его состоянием.
...
Класс служит образцом для создания объектов или, другими словами, объект является экземпляром класса. Класс — это шаблон или чертеж для создания объектов. Он определяет свойства и методы, которые будут у объектов этого класса.
...
Объектно-ориентированное программирование основано на трёх ключевых принципах: *инкапсуляция*, *наследование* и *полиморфизм*.
...
**Инкапсуляция** — это механизм скрытия внутреннего состояния объекта и предоставление доступа к нему только через публичные методы. Это помогает защитить данные от неправильного использования и уменьшить зависимость кода.
...
В ООП данные и методы одного класса могут передаваться другим классам с помощью механизма **наследования**. **Наследование** — это механизм, позволяющий создавать новый класс на основе существующего класса. Новый класс (наследник) наследует свойства и методы базового класса и может добавлять свои собственные.

Порожденный класс (**потомок**), наследующий характеристики другого класса, обладает теми же возможностями, что и класс (**предок**), от которого он порожден. При этом класс-предок остается без изменения, а классу-потомку можно добавлять новые элементы (поля, методы, свойства) или изменять унаследованные методы. Благодаря этому класс-потомок обладает большими возможностями, чем предок. Так, например, все классы порождены от корневого класса `System.Object`.
...
class Program
{
    ...
    {
        Dog dog = new Dog();
        dog.Eat(); // Метод, унаследованный от класса Animal
...
**Полиморфизм** — это способность объектов разных классов обрабатывать вызовы методов с одинаковым именем. Он может быть реализован через переопределение методов и интерфейсы.
...
Классы являются основным строительным блоком в объектно-ориентированном программировании (ООП). В языке C# классы используются для определения объектов, которые содержат данные (свойства) и действия (методы). Классы позволяют создавать типы данных, которые могут объединять данные и методы для работы с этими данными.

**Класс** – это абстрактный тип данных, т.е. это некоторый шаблон, на основе которого будут создаваться его экземпляры – объекты. В Си-шарп классы объявляются с помощью ключевого слова `class`:
```c#
...
#### Определение класса
Класс определяется с помощью ключевого слова `class`, за которым следует имя класса. Внутри класса определяются его члены, такие как свойства, методы, конструкторы и другие.
...
Класс следует объявлять внутри пространства имен `namespace`, но за пределами другого класса (в последних версиях языка разрешено использовать вложенные классы).
...
Основные модификаторы доступа к классам:
...
**Спецификаторы** (модификаторы) определяют свойства класса, а также доступность класса для других элементов программы. Возможные значения спецификаторов перечислены в следующей таблице:
...
2 |	`public` |	Доступ к классу не ограничен (доступ к члену возможен из любого места одной сборки, либо из другой сборки, на которую есть ссылка).
3 |	`protected` |	Доступ только из данного или производного класса (класса-наследника). Используется для вложенных классов.
4 |	`internal` |	Доступ только из данной программы (только из сборки, в которой объявлен данный челн класса).
5 |	`protected internal` |	Доступ только из данного и производного класса и из данной программы (из одной сборки, либо из класса-наследника другой сборки).
...
Не указав модификатор доступа для члена, по умолчанию ему будет присвоен режим `private`. При помощи модификаторов доступа в Си-шарп реализуется один из базовых принципов ООП – инкапсуляция.
...
Классы и члены класса можно сделать статическими с помощью служебного слова `static`.
...
После объявления класса можно создавать объекты при помощи ключевого слова `new` и имени класса:
```c#
namespace HelloWorld
{
  class Student
...
Экземпляр класса создается с использованием ключевого слова `new`, которое вызывает конструктор класса. **Конструктор** — это специальный метод, который вызывается при создании объекта и инициализирует его.
...
1. **Данные** (**поля**): переменные или константы.
...
7. **Операции** (**операторы** — задают действия с объектами с помощью знаков операций).
8. **События** (определяют уведомления, которые может генерировать класс).
9. **Типы** (вложенные типы — типы данных, внутренние по отношению к классу).

Все члены класса также имеют свой уровень доступа. В данном разделе мы рассмотрим первые четыре категории элементов класса
...
**Поле** – это переменная, объявленная внутри класса. Для получения доступа к полям следует использовать свойства или методы.
...
**Свойства класса** — это член класса, он предоставляет удобный механизм доступа к полю класса (чтение поля и запись). При использовании свойства, мы обращаемся к нему, как к полю класса, но на самом деле компилятор преобразовывает это обращение к вызову соответствующего неявного метода. Такой метод называется **аксессором** (**accessor**).

Существует два аксессора: `get` (для получения данных) и `set` (для записи). Объявление простого свойства имеет следующую структуру:
```c#
<тип> <имя>
{
  get
...
Свойства класса позволяют управлять доступом к полям класса. Они могут включать в себя логику для проверки и изменения значений.
...
Методы определяют действия, которые может выполнять объект класса. Они могут возвращать значение или быть процедурными (без возвращаемого значения).
```c#
...
### Конструкторы
Конструкторы используются для инициализации объектов класса. Они могут быть перегружены, что позволяет создавать объекты с разными начальными состояниями. Конструкторы делятся на *конструкторы класса* (для статических классов) и *конструкторы экземпляра* класса (всех остальных классов).
...
### Наследование
Наследование (inheritance) является одним из ключевых моментов ООП. Его смысл состоит в том, что мы можем расширить функциональность уже существующих классов за счет добавления нового функционала или изменения старого. Наследование позволяет создать новый класс на основе существующего. Новый класс (наследник) наследует свойства и методы базового класса, и может добавлять свои собственные.
...
### Наследование конструкторов. Ключевое слово `base`
...
Продолжим рассмотрение примера с классами `Person` и `Employee`. Добавим в наши классы конструкторы:
```c#
class Person
{
...
Кроме конструкторов, мы можем обратиться с помощью ключевого слова `base` к другими членам базового класса. В нашем случае вызов `base.Display();` будет обращением к методу `Display()` в классе `Person`.
...
### Полиморфизм: виртуальные методы, переопределение методов в прозиводных классах
Полиморфизм позволяет объектам обрабатывать вызовы методов с одинаковым именем по-разному в зависимости от их типа. Это достигается с помощью переопределения методов в производных классах.

Сейчас класс `Employee` реализует всю ту же функциональность, что и класс `Person`. Но что, если мы вдруг захотели, чтобы в классе `Employee` наряду с именем и фамилией выводилась бы информация о компании, в которой работает данный сотрудник. Для этого метод, который надо переопределить, в базовом классе помечается модификатором `virtual`, а при переопределении в производном классе — модификатором `override`:
```c#
class Person
{
  public string FirstName { get; set; }
...
Мы бы также можем запретить переопределение методов и свойств. В этом случае их надо объявлять с модификатором `sealed`:
```c#
class Person
{
  public string FirstName { get; set; }
...
Абстрактный класс похож на обычный класс. Он также может иметь переменные, методы, конструкторы, свойства. Но мы не можем создать объект или экземпляр абстрактного класса. Абстрактные классы лишь предоставляют базовый функционал для классов-наследников. А производные классы уже реализуют этот функционал.
...
class Client : Person
{
  ...
  public Client(string lName, string fName, string comp) : base(fName, lName)
  {
    Bank = comp;
  }
  public override void Display()
...
https://ru.wikipedia.org/wiki/%D0%AD%D0%BA%D1%81%D0%BF%D0%BE%D0%BD%D0%B5%D0%BD%D1%86%D0%B8%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0%D1%8F_%D0%B7%D0%B0%D0%BF%D0%B8%D1%81%D1%8C
...
Любое данное число может быть записано в виде $M \cdot 10^p$ многими путями; например 350 может быть записано как $3,5 ⋅ 10^2$ или $35 ⋅ 10^1$.
...
> В инженерной нормализованной записи (в том числе в информатике) мантисса обычно выбирается в пределах $0,1 < |a| ⩽ 1: 350 = 0,35 ⋅ 10^3$.
...
- M — мантисса,
- E — экспонента (от англ. «exponent», показатель степени), означающая «·10^» («…умножить на десять в степени…»),
- p — порядок.
Например:

$$
1,602176565\text{E-}19 = 1,602176565 ⋅ 10^{−19} \text{(элементарный заряд в Кл)}; \\
1,380650424\text{E-}23 = 1,380650424 ⋅ 10^{−23} \text{(постоянная Больцмана в Дж/К)};\\
6,02214129\text{E}23 = 6,02214129 ⋅ 10^{23} \text{(число Авогадро)}.
$$
...
{\displaystyle {\text{6,02214129e23}}}$
...
Метод `public int Next()` при каждом вызове возвращает положительное целое, равномерно распределенное в некотором диапазоне. Диапазон задается параметрами метода. Три реализации метода отличаются набором параметров:
